import { Order, OrderDetail, MyOrderItem, MenuCategory, MenuProduct, MenuVariation } from './dinbendon';
...
export function buildCategoryKeyboard(
  ...
  const buttons = categories.map((cat, i) => [
    { text: `${cat.name} (${cat.products.length})`, callback_data: `category:${i}` },
  ]);
...
export function formatProductList(shopName: string, categoryName: string): string {
...
export function buildProductKeyboard(
  ...
  const buttons = products.map((p) => {
    ...
    return [{ text: `${truncate(p.name, 24)} ${priceStr}`, callback_data: `product:${p.id}` }];
  });
...
export function buildVariationKeyboard(
  ...
  const buttons = variations.map((v) => {
    const label = v.name ? `${v.name} - $${v.price}` : `${productName} - $${v.price}`;
...
export function buildQuantityKeyboard(): { text: string; callback_data: string }[][] {
  return [
    [1, 2, 3, 4, 5].map((q) => ({ text: String(q), callback_data: `qty:${q}` })),
...
export function buildMyOrderKeyboard(items: MyOrderItem[]): { text: string; callback_data: string }[][] {
